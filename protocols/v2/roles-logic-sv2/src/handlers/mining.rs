//! # Mining Handlers
//!
//! This module defines traits and functions for handling mining-related messages within the Stratum V2 protocol.
//!
//! ## Core Traits
//!
//! - `ParseUpstreamMiningMessages`: Implemented by downstream nodes to process mining messages received from upstream nodes.
//!   This trait provides methods for handling mining events like new mining jobs, share submissions, extranonce prefix updates, and channel status updates.
//! - `ParseDownstreamMiningMessages`: Implemented by upstream nodes to manage mining messages received from downstream nodes.
//!   This trait includes methods for managing tasks such as submitting shares, opening mining channels, and handling mining job responses.
//!
//! ## Message Handling
//!
//! Handlers in this module are responsible for:
//! - Parsing and deserializing mining-related messages into the appropriate types.
//! - Dispatching the deserialized messages to specific handler functions based on message type, such as handling new mining jobs, share submissions, and extranonce updates.
//! - Ensuring the integrity and validity of received messages, while interacting with downstream mining systems to ensure proper communication and task execution.
//!
//! ## Return Type
//!
//! Functions return `Result<SendTo<Down>, Error>`, where `SendTo<Down>` specifies the next action for the message: whether it should be sent to the downstream node, an error response should be generated, or the message should be ignored.
//!
//! ## Structure
//!
//! This module includes:
//! - Traits for processing mining-related messages for both upstream and downstream communication.
//! - Functions to parse, deserialize, and process messages related to mining, ensuring robust error handling for unexpected conditions.
//! - Support for managing mining channels, extranonce prefixes, and share submissions, while handling edge cases and ensuring the correctness of the mining process.
    ...
    Self: IsMiningDownstream + Sized + D,
{
    ...
    ///
    /// This method processes a `SetCustomMiningJob` message and applies the custom mining job settings.
    ///
    /// # Arguments
    ...
    Self: IsMiningUpstream<Down, Selector> + Sized + D,
{
    ...
    ///
    /// # Returns
    /// - `Option<Arc<Mutex<RequestIdMapper>>>`: An optional RequestIdMapper for request ID modification.
    fn get_request_id_mapper(&mut self) -> Option<Arc<Mutex<RequestIdMapper>>> {
        None
    }

    /// Parses and routes SV2 mining messages from the upstream based on the message type and payload.
    /// The implementor of DownstreamMining needs to pass a RequestIdMapper if changing the request ID.
    /// Proxies typically need this to ensure the request ID is unique across the connection.
    ///
    /// # Arguments
    /// - `self_mutex`: The `Arc<Mutex<Self>>` representing the downstream entity.
    ...
    ///
    /// # Returns
    /// - `Result<SendTo<Down>, Error>`: The result of processing the message, either sending a response or an error.
    fn handle_message_mining(
        self_mutex: Arc<Mutex<Self>>,
        message_type: u8,
        payload: &mut [u8],
    ...
    /// Handles the deserialized mining message from the upstream, processing it according to the routing logic.
    ///
    /// # Arguments
    /// - `self_mutex`: The `Arc<Mutex<Self>>` representing the downstream entity.
