//! This module contains the routing logic used by handlers to determine where a message should be relayed or responded to.
//!
//! ## Overview
//!
//! The routing logic defines a set of traits and structures to manage message routing in Stratum V2.
...
//! - **`CommonRoutingLogic`**: Enum defining the various routing logic for the common protocol (e.g., Proxy, None).
//! - **`MiningRoutingLogic`**: Enum defining the routing logic for the mining protocol (e.g., Proxy, None).
//! - **`NoRouting`**: Marker router that implements both `CommonRouter` and `MiningRouter` for cases where no routing logic is required.
//! - **`MiningProxyRoutingLogic`**: Routing logic valid for a standard Sv2 mining proxy, implementing both `CommonRouter` and `MiningRouter`.
//!
//! ## Details
//!
//! ### Traits
...
//! - **`MiningRouter`**: Defines routing behavior for mining protocol messages. Requires `DownstreamMiningSelector` for downstream selection.
//!
//! ### Enums
//!
//! - **`CommonRoutingLogic`**: Represents possible routing logics for the common protocol, such as proxy-based routing or no routing.
//! - **`MiningRoutingLogic`**: Represents possible routing logics for the mining protocol, supporting additional parameters such as selectors and marker traits.
//!
//! ### Structures
//!
//! - **`NoRouting`**: A minimal implementation of `CommonRouter` and `MiningRouter` that panics when used. Its primary purpose is to serve as a placeholder for cases where no routing logic is applied.
//! - **`MiningProxyRoutingLogic`**: Implements routing logic for a standard Sv2 proxy, including upstream selection and message transformation.
//!
//! ## Future Work
//!
...
/// [`crate::handlers::common::ParseDownstreamCommonMessages`]) to determine the behavior for common protocol routing.
...
/// [`crate::handlers::mining::ParseDownstreamMiningMessages`]) to determine the behavior for mining protocol routing.
/// This trait extends [`CommonRouter`] to handle mining-specific routing logic.
...
> CommonRouter for MiningProxyRoutingLogic<Down, Up, Sel>
{
    ...
    ///
    /// # Returns
    /// - `Result<(CommonDownstreamData, SetupConnectionSuccess), Error>`: On success, returns the downstream
    ///   connection data and the corresponding setup success message. Returns an error otherwise.
    fn on_setup_connection(
        &mut self,
        message: &SetupConnection,
    ) -> Result<(CommonDownstreamData, SetupConnectionSuccess), Error> {
...
> MiningRouter<Down, Up, Sel> for MiningProxyRoutingLogic<Down, Up, Sel>
{
    ...
    ///
    /// # Returns
    /// - `Result<Arc<Mutex<Up>>, Error>`: Returns the selected upstream for the downstream or an error.
    fn on_open_standard_channel(
        &mut self,
        downstream: Arc<Mutex<Down>>,
        request: &mut OpenStandardMiningChannel,
    ...
    ///
    /// # Returns
    /// - `Result<Arc<Mutex<Down>>, Error>`: Returns the downstream corresponding to the request or an error.
    fn on_open_standard_channel_success(
        &mut self,
        upstream: Arc<Mutex<Up>>,
        request: &mut OpenStandardMiningChannelSuccess,
...
{
    ups.iter()
        .filter(|up_mutex| match up_mutex.safe_lock(|up| !up.is_header_only()) {
