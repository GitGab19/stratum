...
impl Sv1Server {
    ...
    pub async fn open_extended_mining_channel(
        ...
        self.sv1_server_data.super_safe_lock(|data| {
            if data.initial_target.is_none() {
                data.initial_target = Some(initial_target.clone());
            }
        });
    ...
    async fn handle_set_target_without_vardiff(
        &self,
        set_target: roles_logic_sv2::mining_sv2::SetTarget<'_>,
    ) {
...
mod tests {
    ...
    use roles_logic_sv2::{mining_sv2::Target, utils::hash_rate_to_target};
    ...
    fn create_test_config() -> TranslatorConfig {
        ...
        let difficulty_config = DownstreamDifficultyConfig::new(100.0, 5.0, true, 0, 0);
        let downstream_config =
            crate::config::DownstreamConfig::new("0.0.0.0".to_string(), 3333, difficulty_config);
    ...
    async fn test_handle_set_target_without_vardiff_aggregated() {
        ...
        let set_target = roles_logic_sv2::mining_sv2::SetTarget {
            channel_id: 1,
