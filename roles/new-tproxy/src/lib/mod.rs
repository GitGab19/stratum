...
// Re-export upstream_sv2 types
pub use upstream_sv2::{
    Message, StdFrame, EitherFrame,
    Upstream,
};

// Re-export roles_logic_sv2 types
pub use roles_logic_sv2::{
    handlers::mining::{SendTo, ParseMiningMessagesFromUpstream},
    parsers::Mining,
};
...
// Example usage of Bridge with Upstream
pub async fn start_proxy(upstream: Arc<Mutex<Upstream>>) -> Result<(), error::Error<'static>> {
    let bridge = proxy::Bridge::new(upstream);
    bridge.start().await
}
